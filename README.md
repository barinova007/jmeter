# jmeter
```
JMeter предоставляет очень широкие возможности по созданию сценариев, я рассмотрю только необходимый 
минимум для тестирования Flex-приложений.
...
Это необходимо, чтобы все запросы между собой были связаны в одну сессию. 
Иначе сервер откажется обрабатывать второй и последующие запросы. 
По одному перетаскиваем сохраненные запросу в Thread Group и добавляем их как детей (Add as Child).
Вместо перетаскивания можно сделать Cut&Paste;
Thread Group → Add → View result tree добавляем слушателя запросов, который просто отображает запрос-ответ.
Сценарий запускается через главное меню Run → Start. 
В соотвествующих элементах (View Results Tree) вы увидите посланные запросы и ответы на них.
Запросы будут отосланы один раз в одном потоке. Количество потоков и циклов задается в настройках Thread Groups:

Number of threads (users) - сколько одновременных потоков обращается к серверу
(количество работающих пользователей);
Ramp-Up Period: время, в течении которого будут запущены все потоки. 
Например, вы делаете тест в 20 потоков и ставите ramp-up period в 60 секунд. 
...
При тестировании нужно указать правильное значение с тем, чтобы каждый пользователь
(поток Thread Group) имел свой уникальный номер клиента. 
...
После этого при отправки запросов, они будут иметь правильный DSid. 
Переменные живут в рамках одного потока Thread Group (одного пользователя). 
...
Тогда можно все запросы связанные с иницализаций (например, ping-запрос и запрос на аутентификацию)
поместить в логический контроллер Once Only Controller.
