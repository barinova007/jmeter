...
Как правило, пользователи работают с приложением в несколько шагов. Например, это может быть вход на первую страницу, выбор товара, добавление в корзину и оформление заказа. Каждый такой сценарий в JMeter соответствует группе запросов - Thread Group в плане тестирования (Test Plan). 
В каждой группе могут быть различные элементы - параметры выполнения, запросы, представление результатов выполнения.
На текущий момент у нас есть только запросы, собранные с помощью HTTP Proxy Server. Создадим на их основе сценарий тестирования (все добавление элементов в JMeter'ом преимущественно осуществляется с помощью контекстного меню дерева элементов, поэтому ищите команды в них).
...
При тестировании нужно указать правильное значение с тем, чтобы каждый пользователь (поток Thread Group) имел свой уникальный номер клиента. Для этого добавим пост-обработчик сообщения XPath Extractor к ping-запросу как показано на рисунке. 
Этот элемент извлекает из полученного сообщения нужную нам строку и сохраняется ее в пользовательской переменной. После этого при отправки запросов, они будут иметь правильный DSid. Переменные живут в рамках одного потока Thread Group (одного пользователя). 
То есть если мы запустим 5 параллельных потоков, в каждом из них DSid будет иметь свое значение. Таким образом, теперь можно запустить много-много потоков и сделать настоящее нагрузочное тестирование.
...
То есть пользователь зашел и начал делать какие-то операции. Вам интересно, будет происходит утечка памяти или что-то подобное. 
...
Элементы в нем будут выполнены в потоке только один раз (сами поток будет выполнен Loop count раз). Можно пойти по другому пути и объединить логику работы в Loop Controller.
